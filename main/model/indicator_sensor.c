...
static void
__sensor_history_data_get(struct sensor_history_data *p_history, struct view_data_sensor_history_data *p_data) {
...
static void __sensor_history_data_save(void) {
    xSemaphoreTake(__g_data_mutex, portMAX_DELAY);
    esp_err_t ret = 0;
    ret = indicator_storage_write(SENSOR_HISTORY_DATA_STORAGE, (void *) &__g_sensor_history_data,
                                  sizeof(struct indicator_sensor_history_data));
    xSemaphoreGive(__g_data_mutex);
...
static void
__sensor_history_data_day_check(struct sensor_data_average p_data_day[], const char *p_sensor_name, time_t now) {
    ...
    for (int i = 0; i < 24; i++) {
        if (p_data_day[i].valid) {
            ESP_LOGI(TAG, "%s index:%d, data:%.0f, time:%d", p_sensor_name, i, p_data_day[i].data,
                     p_data_day[i].timestamp);
        }
    }
...
static void
__sensor_history_data_week_check(struct sensor_data_minmax p_data_week[], const char *p_sensor_name, time_t now) {
    ...
    for (int i = 0; i < 7; i++) {
        if (p_data_week[i].valid) {
            ESP_LOGI(TAG, "%s, index:%d, min:%.0f, max:%.0f, time:%d", p_sensor_name, i, p_data_week[i].min,
                     p_data_week[i].max, p_data_week[i].timestamp);
        }
    }
...
static void __sensor_history_data_day_insert(struct sensor_data_average p_data_day[], struct sensor_present_data *p_cur,
                                             time_t now) {
    int history_hour = 0;
    int cur_hour = 0;
    struct tm timeinfo;
...
static void
__sensor_history_data_week_insert(struct sensor_data_minmax p_data_week[], struct sensor_present_data *p_cur,
                                  time_t now) {
    int history_mday = 0;
    int cur_mday = 0;
    struct tm timeinfo;
...
static void __sensor_history_data_week_update(time_t now) {
    ...
    __sensor_history_data_week_insert(__g_sensor_history_data.humidity.data_week, &__g_sensor_present_data.humidity,
                                      now);
...
static void __view_event_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data) {
    ...
    switch (id) {
        case VIEW_EVENT_SENSOR_TEMP_HISTORY: {
            ...
            data.sensor_type = SENSOR_DATA_TEMP;
            data.resolution = 1;
            esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_SENSOR_DATA_HISTORY, &data,
                              sizeof(struct view_data_sensor_history_data), portMAX_DELAY);
            break;
        }
