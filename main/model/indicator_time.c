...
static void __time_cfg_print(struct view_data_time_cfg *p_cfg) {
    printf("time_format_24:%d, auto_update:%d, time:%d, auto_update_zone:%d, zone:%d, daylight:%d\n", \
      (bool) p_cfg->time_format_24, (bool) p_cfg->auto_update, (long) p_cfg->time, (bool) p_cfg->auto_update_zone,
           (int8_t) p_cfg->zone, (bool) p_cfg->daylight);
}
...
static void __time_sync_notification_cb(struct timeval *tv) {
    ...
    esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_TIME, &time_format_24, sizeof(time_format_24),
                      portMAX_DELAY);
}
...
static void __view_event_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data) {
    switch (id) {
        case VIEW_EVENT_TIME_CFG_APPLY: {
            ...
            esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_TIME, &time_format_24,
                              sizeof(time_format_24), portMAX_DELAY);
            break;
        }
