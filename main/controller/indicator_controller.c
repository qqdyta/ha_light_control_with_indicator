...
static void __brighness_cfg_event_cb(lv_event_t *e) {
    ...
    esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_BRIGHTNESS_UPDATE, &vaule, sizeof(vaule),
                      portMAX_DELAY);
}
...
static void __display_cfg_apply_event_cb(lv_event_t *e) {
    ...
    esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_DISPLAY_CFG_APPLY, &cfg, sizeof(cfg),
                      portMAX_DELAY);
}
...
static void ui_event_sensor_humidity_chart(lv_event_t *e) {
    ...
    lv_obj_t *cur_screen = lv_scr_act();
    if (event_code == LV_EVENT_CLICKED && cur_screen == ui_screen_sensor) {
        esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_SENSOR_HUMIDITY_HISTORY, NULL, 0,
                          portMAX_DELAY);
        _ui_screen_change(ui_screen_sensor_chart, LV_SCR_LOAD_ANIM_OVER_LEFT, 200, 0);
    }
}
